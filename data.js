...
const OVERVIEW_FACTS = [
  ["여행 목적", "유태현 · 이채린 신혼여행(허니문) — 파리·스위스·이탈리아를 도는 신혼 첫 장기 여행"],
  ["기간", "2026년 11월 15일(일) 인천 출발 ~ 2026년 11월 25일(수) 인천 도착 (총 11일, 9박)"],
  ["방문국", "프랑스(파리) → 스위스(루체른·그린델발트) → 이탈리아(피렌체·로마)"],
  ["인원", "신랑신부 2인"],
  ["여행 형태", "블루여행사 패키지 — 왕복 국제선(OZ) + 호텔 9박(조식포함) + 스위스패스 4일권 + 현지 픽업·투어"],
  ["숙소 등급", "3~4성급 호텔, 파리·그린델발트는 허니문 업그레이드(Eiffel View / Eiger View) 적용"],
  ["이동 수단", "파리-바젤-루체른-그린델발트-밀라노-피렌체-로마 전 구간 철도, 이탈리아 2구간은 1등석 업그레이드"],
  ["예상 총예산", "성인 2인 패키지 견적가 11,330,000원 (1인 5,665,000원, 2026-06-10 견적 기준, 항공TAX·유류할증료 포함)"],
];

// ---- 설계 원칙 / 핵심 안내 ----
const PRINCIPLES = [
  "허니문 이벤트 혜택(2026년 한시) — 다이슨 헤어드라이어 또는 1인 20만원 할인 중 택1, 호텔 객실 업그레이드, USB 멀티 어댑터·커플 네임택 제공. 최초 견적 확인 후 3일 이내 계약 시 적용됩니다.",
  "파리에서는 에펠탑 뷰 객실, 그린델발트에서는 아이거 뷰 객실로 업그레이드되어 있어 전망 좋은 저녁 시간을 계획해두면 좋습니다.",
  "스위스패스 4일권으로 주간열차·유람선·대중교통이 무료이니, 그린델발트·루체른 구간에서는 이동을 걱정하지 않아도 됩니다.",
  "이탈리아 밀라노→피렌체, 피렌체→로마 구간은 1등석으로 업그레이드되어 있어 장거리 구간에서도 편하게 이동합니다.",
  "융프라우 등반열차 비용과 호텔 투어리스트TAX(1박/1인 3~10유로)는 불포함이므로 현지에서 별도 준비가 필요합니다.",
  "여행사 24시간 비상연락서비스가 제공되니 현지에서 문제가 생기면 담당자(배영은 과장)나 비상연락처로 바로 연락하면 됩니다.",
];
